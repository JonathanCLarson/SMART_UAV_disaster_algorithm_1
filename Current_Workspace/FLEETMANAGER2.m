classdef FLEETMANAGER2
    ...
    properties
        ...
        UAV % UAV objects that store: [remainingbatterylife(hrs), assignedrequestid, timetocomplete, speed, payloadcapacity(kg) remainingpayload x y] 
        HPAssignment % list of UAVs assigned to HP request
        LPAssignment % list of UAVs assigned to LP requests
    end
    
    methods
        % Constructor: Initialize all properties and plot locations of
        % bases and delivery zones
        function obj = fleetmanager2(obj,RR,FS,NB,BL,ND,DL, HPA,LPA)
