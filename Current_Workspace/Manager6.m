...
%                         lowUAVs(length(lowUAVs)+1)=obj.uavList(c); 
            ...
            %
            requestList = Manager6.sortRequests(requestList);
...
%                     disp('Empty request found')
                ...
                if Distance(UAVs(c).position,UAVs(c).request.zone.position)<.0001 
                    % Have drones at the base wait for the next time step
                    if UAVs(c).request.priority == 'B'
                        UAVs(c).idleCounter = 1;
                    else
                        % Have drones assigned to a request at their current
                        % location instantly deliver
                        UAVs(c).deliver();
                    end
                end
            end  
            ...
            if cargoTest == 1
                ...
                if ((length(uavList) == 1))                
                    ...
                    if(Manager6.checkRange(uavList(1), requestList(bestInd)))
                        ...
                        if(~isempty(uavList(1).request)&& uavList(1).request.zone.ID~=requestList(bestInd).zone.ID&& uavList(1).request.status~='B')
