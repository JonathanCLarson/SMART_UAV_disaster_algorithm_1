...
classdef Manager5 < handle
    ...
    methods
        ...
        function obj = Manager5(zones, base)
            obj.requestZones = zones;
            obj.time = 0;
            obj.uavList = UAVDrone5.empty;
            obj.base = base;
            obj.completedList = Request5.empty;
            obj.expiredList = Request5.empty;
            obj.requestsMet = 0;
            obj.expired = 0;
            obj.numRedirect = 0;
            obj.probCrash=0;
       ...
       % Function to refresh the UAV's and requests
        ...
        function assign(obj)
            ...
            hiUAVs = UAVDrone5.empty; % UAVs with only high priority cargo
            lowUAVs = UAVDrone5.empty; % UAVs with only low priority cargo
            for c = 1:length(obj.uavList)
                ...
                if(obj.uavList(c).idleCounter==0)
                    if isempty(obj.uavList(c).cargo)
                        % Empty UAV
                        obj.uavList(c).request = obj.uavList(c).base.activeList;
                    elseif(contains(obj.uavList(c).cargo,'H'))
                        % Has high cargo
                        hiUAVs(length(hiUAVs)+1)=obj.uavList(c);                        
                    elseif contains(obj.uavList(c).cargo,'L')
                        % Only has low cargo
                        lowUAVs(length(lowUAVs)+1)=obj.uavList(c); 
                    
                    end
                end
            end
            numDrones = length(lowUAVs)+length(hiUAVs);
%           requestList = Request5.empty;
            highReqs = Request5.empty; % Array of high priority requests
            lowReqs = Request5.empty; % Array of low priority requests
            ...
            if (~isempty(highReqs)|| ~isempty(lowReqs))
                ...
                if (~isempty(highReqs)&& ~isempty(hiUAVs))
                    %disp('Choosing H Requests')
                    leftoverUAVs =Manager5.chooseRequests(highReqs,hiUAVs);
                ...
                if (~isempty(lowReqs)&&~isempty(lowUAVs))
                    %disp('Choosing L Requests')
                    Manager5.chooseRequests(lowReqs,lowUAVs);
                end
            else
            ...
            for c=1:length(obj.uavList)
                if(isempty(obj.uavList(c).request))
%                     disp('Empty request found')
                    %disp(obj.uavList(c))
                    %disp(obj.uavList(c).base.activeList)
                    obj.uavList(c).request=obj.uavList(c).base.activeList(1);
                end
                % Have drones at the base wait for the next time step
                if Distance(obj.uavList(c).position,obj.uavList(c).base.position)<.0001 && obj.uavList(c).request.priority == 'B'
                     obj.uavList(c).idleCounter=1;
                end
            end  
...
%            end
%            nextRequest = Manager5.chooseRequest(activeArray,uav);
...
%         end
    ...
    methods (Static)
        ...
        function unAssigUAV=chooseRequests(requestList,uavList) 
            ...
            unAssigUAV=UAVDrone5.empty; % Stores the UAV's that haven't been given requests yet
            hdArray=[]; % Array of humanitarian distances to compare
            reqArray=Request5.empty; % Array of requests for which the current UAV minimizes the HD
            ...
            if(length(uavList) == 1)
               ...
               % disp('Assigning single UAV to:')
               % disp(requestList(index))
                if(Manager5.checkRange(uavList(1), requestList(index)))
                    % Count redirected UAV's
                    if(~isempty(uavList(1).request)&& uavList(1).request.zone.ID~=requestList(index).zone.ID&& uavList(1).request.status~='B')
                    ...
                    uavList.request = requestList(index);
                     ...
                     uavList.request = uavList.base.activeList;
                end
            ...
            %   then choose the best request for each UAV to service
                ...
                for u=1:length(uavList)
                    ...
                    if(Manager5.checkRange(uavList(u), reqArray(bestFit)))
                        ...
                        uavList(u).request = uavList(u).base.activeList;
                    end
                    % RESET arrays before moving to next UAV
                    reqArray=Request5.empty;
                    hdArray=[];
                    end
                end
                ...
                if(~isempty(unAssigUAV)&&~isempty(unAssigReq))
                    Manager5.chooseRequests(unAssigReq,unAssigUAV);
