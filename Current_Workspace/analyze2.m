function  [numComp, perComp, numExp,wait,waitHi, recharge,restock,refill,idleTotal] = analyze2(manager)
...
%   input: manager = UAV manager object to analyze
    ...
    for c=1:length(manager.requestZones)
        for k=1:length(manager.requestZones(c).activeList)
            this = manager.requestZones(c).activeList(k); % temporary variable
            if this.status==1
                numHi=numHi+1;
                waitTimeHi = waitTimeHi + this.timeElapsed;
            end
            waitTime=waitTime+this.timeElapsed;
            numReq=numReq+1;
