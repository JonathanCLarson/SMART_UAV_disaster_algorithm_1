    ...
    methods
       ...
       % Function to refresh the UAV's and requests
        ...
        function assign(obj)
            ...
            hiUAVs = UAVDrone6.empty; % UAVs with only high priority cargo
...
%           requestList = Request6.empty;
            highReqs = Request6.empty; % Array of high priority requests
            lowReqs = Request6.empty; % Array of low priority requests
            ...
            if (~isempty(highReqs)|| ~isempty(lowReqs))
                ...
                if (~isempty(highReqs)&& ~isempty(hiUAVs))
                    ...
                    leftoverUAVs =Manager6.chooseRequests(highReqs,hiUAVs);
                ...
                if (~isempty(lowReqs)&&~isempty(lowUAVs))
                    ...
                    Manager6.chooseRequests(lowReqs,lowUAVs);
                end
            else
        ...
        function unAssigUAV=chooseRequests(requestList,uavList) 
            ...
            if(length(uavList) == 1)
               ...
               % disp(requestList(index))
                if(Manager6.checkRange(uavList(1), requestList(index)))
            ...
            %   then choose the best request for each UAV to service
                ...
                for u=1:length(uavList)
                    ...
                    if(Manager6.checkRange(uavList(u), reqArray(bestFit)))
